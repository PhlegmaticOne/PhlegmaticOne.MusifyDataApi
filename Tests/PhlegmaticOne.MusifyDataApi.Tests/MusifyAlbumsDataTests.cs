...
public class MusifyAlbumsDataTests
{
    private readonly IMusifyAlbumsData _musifyTracksData;
    public MusifyAlbumsDataTests()
    {
        var serviceCollection = new ServiceCollection();
        serviceCollection.AddMusifyDataApi();
        var serviceProvider = serviceCollection.BuildServiceProvider();

        _musifyTracksData = serviceProvider.GetRequiredService<IMusifyAlbumsData>();
    }
    [Fact]
    ...
    public async Task GetAlbumWithTracksInfoAsync_ShouldBeExpectedAlbumData_Test()
    {
        ...
        Assert.Collection(data.Genres,
            g => Assert.Equal("Psychedelic Rock", g),
            g => Assert.Equal("Occult Rock", g));
    ...
    [Fact]
    public async Task GetAlbumWithTracksDataAsync_ShouldHaveTrackData_Test()
    {
        var url = "https://musify.club/release/vinterriket-aura-2003-83280";
        var result = await _musifyTracksData.GetAlbumWithTracksDataAsync(url);
        var data = result.Data!;

        Assert.All(data.Tracks, t =>
        {
            Assert.NotNull(t.TrackData);
            Assert.NotEmpty(t.TrackData);
        });
    }
